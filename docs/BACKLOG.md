...
9. ✅ **3.16 `purchase_cost`** (2026-09-12, PR #24) — เพิ่ม field `purchase_cost` (nullable, min 0) ใน `productModel` + `productService.createProduct`/`updateProduct` (validate ≥0 เหมือน `sale_price`) · `recipeService.getUnitCostByProduct` fallback ไปใช้ `purchase_cost` เมื่อสินค้าไม่มีสูตร (หรือมีแต่ `yield_qty=0`) — ลำดับความสำคัญ: สูตรมาก่อนเสมอ → `purchase_cost` → `null` · เทส: `tests/integration/getUnitCostByProduct.test.ts` (4 เคส)
