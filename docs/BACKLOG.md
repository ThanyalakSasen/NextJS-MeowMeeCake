...
| 2.8 | ยกเลิกออเดอร์ที่จ่ายเงินแล้ว ไม่ผูกกับการคืนเงิน | `src/services/orderService.ts` `updateOrderStatus` (สาขา cancel, ~บรรทัด 520) | `updateOrderStatus` สาขา cancel คืนสต็อก + คืนสิทธิ์โปร + set cancelled — **ไม่ดู `payment_status`** · `refundPayment` กับ cancel order เป็นคนละทางที่ไม่เชื่อมกัน (refund ไม่ set order เป็น cancelled, cancel ไม่ refund) · 🟡 **ทำแล้วบางส่วน (2026-09-07):** ฝั่งลูกค้า `cancelOrder({ allowedFrom })` เพิ่มเช็ค `payment_status === "paid"` → `conflict` 409 "ติดต่อร้านเพื่อขอยกเลิกและคืนเงิน" (กันลูกค้ายกเลิกออเดอร์ `confirmed` ที่จ่ายแล้วเงียบ ๆ) · **ยังไม่ทำ:** ฝั่งแอดมิน — ยกเลิกออเดอร์ `paid` ผ่าน `/api/admin/orders/[id]/status` ยังไม่สร้าง refund record / ไม่เรียก `refundPayment` อัตโนมัติ (แอดมินต้องกดคืนเงินแยก) · รายละเอียด → [`order-cancel.md`](order-cancel.md) §3 |
| 2.9 | โปรโมชัน `usage_limit` / `max_user_per_user` ไม่ atomic | `src/services/promotionService.ts` `validateForOrder` (บรรทัด ~212–223) + `src/services/promotionUsageService.ts` `recordUsage` (บรรทัด ~40–43) | `validateForOrder` อ่าน `promo.used_count` แล้วเทียบ `usage_limit` (read) → `recordUsage` ค่อย `$inc used_count` ทีหลัง (best-effort, ไม่มี guard) → 2 ออเดอร์พร้อมกันผ่าน check ทั้งคู่ → `used_count` เกินลิมิต · `max_user_per_user` race แบบเดียวกัน (`getUserUsageCount` นับ row) · วิธีแก้: ใช้ `findOneAndUpdate({ _id, $expr: { $lt: ["$used_count", "$usage_limit"] } }, { $inc: { used_count: 1 } })` แบบเดียวกับ preorder quota (§8) แล้วถือผลลัพธ์เป็น "ได้สิทธิ์" |
| 2.10 | สร้าง payment ซ้ำได้หลายใบต่อ 1 ออเดอร์ | `src/services/paymentService.ts` `createPayment` (บรรทัด ~78–121) | เช็คแค่ `order.payment_status === "paid"` → conflict · ไม่กันการสร้าง payment ใหม่ขณะที่ยังมีใบ `pending` ค้าง → เกิด payment หลาย doc ต่อ order เดียว, `order.payment_id` ถูกเขียนทับเป็นใบล่าสุด (บรรทัด 113), ใบ pending เก่ากลายเป็น orphan (ยังชี้ `order_id`) → แอดมิน verify ใบเก่าได้ · วิธีแก้: ก่อนสร้าง ให้ soft-delete / reuse ใบ `pending` เดิมของออเดอร์นั้น หรือทำ partial unique `{ order_id } where status:"pending", deleted_at:null` |
| 2.11 | `discount_amount = 0` ยังผูก `promotion_id` กับออเดอร์ | `src/services/orderService.ts` `persistOrder` (บรรทัด 332 + 355) | เวอร์ชันทั่วไปของ 2.6 — โปร `Amount`/`Percentage` ที่คิดออกมาได้ 0 (discount_value เล็ก / eligibleAmount = 0) → `order.promotion_id` ถูกเซ็ต แต่ guard `discount_amount > 0` ทำให้ไม่ `recordUsage` → ออเดอร์ผูกโปรโดยไม่มี usage record · วิธีแก้ (เลือก 1): ไม่เซ็ต `order.promotion_id` เมื่อ `discount_amount === 0`, หรือ ย้าย guard ให้ `recordUsage` ทุกครั้งที่มี `appliedPromotion` (ถือว่าใช้สิทธิ์ไปแล้ว) |
...
| 3.2 | ไม่มี rate-limit ที่ `/api/auth/login` | มี account lockout (5 ครั้ง / 15 นาที ต่อบัญชี) แต่ไม่มี IP throttle |
| 3.3 | `src/lib/compensation.ts` + `src/lib/logger.ts` ยังไม่ได้สร้าง | best-effort ยัง inline try/catch, `console.error` กระจาย |
| 3.4 | ไม่มี test | แนะนำ vitest + `mongodb-memory-server` เริ่มที่ `discountEngine`, `orderService`, `ingredientTransactionService`, `productCode`, `upload` |
...
| 3.6 | ไม่มี eslint config | `next build` เลย skip lint; คอมเมนต์ `/* eslint-disable */` ไม่มีผล |
...
| 3.9 | Google login = ID token flow เท่านั้น | `src/services/authService.ts` `loginWithGoogle` รับ credential (ID token) จาก Google Identity Services — ไม่ใช่ redirect/code flow ตามที่ `GOOGLE_CALLBACK_URL` ใน env บอกใบ้ |
| 3.10 | CORS / CSRF | ถ้า frontend คนละ origin ต้องตั้ง CORS; CSRF ตอนนี้พึ่ง cookie `SameSite=Lax` อย่างเดียว |
