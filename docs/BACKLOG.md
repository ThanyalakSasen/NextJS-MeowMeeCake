...
| 3.4 | ✅ unit + integration + CI (2026-09-10 → 2026-09-12) | vitest **2 projects**: `unit` (`tests/lib/`, ไม่ต่อ DB) + `integration` (`tests/integration/`, `mongodb-memory-server`) · scripts: `test` / `test:integration` / `test:all` / `typecheck:test` · **รอบ 4c ปิดครบ** `cartService`/`ingredientTransactionService`(`createTransaction`)/`deliveryService.quoteForCart` ที่เคยขาด (รายละเอียด → รอบ 4c ข้อ 10 ด้านบน) · ✅ **CI** `.github/workflows/ci.yml` (PR #13) รัน `typecheck → typecheck:test → lint → test → test:integration → build` ทุก push+PR · `tests/` exclude จาก `tsconfig.json` หลัก → `tsconfig.test.json` · **ตัวเลขล่าสุด (หลังรอบ 5 เสร็จ):** unit **171** · integration **125** · → [`infra-tooling.md`](infra-tooling.md) §3 |
...
| 3.8 | ~~สมุดที่อยู่ไม่เชื่อม checkout~~ | ✅ แก้แล้ว (2026-09-12) — `address_id`/`recipient_name`/`recipient_phone` + `addressService.resolveDeliverySnapshot()` (รายละเอียด → รอบ 4c ข้อ 7 ด้านบน) · **ยังไม่ทำ (gap ที่รู้ตัว):** `/api/shop/preorders` (POST) ยังเป็น path คู่ขนานที่ไม่รองรับ `address_id` เลย |
...
| 3.12 | ~~จุดต่อ LINE (`src/lib/notify.ts`)~~ | ✅ **ล้าสมัยแล้ว ไม่ต้องทำ** (2026-09-12) — ระบบแจ้งเตือนตัวจริงถูกสร้างเสร็จก่อนหน้านี้แล้ว (`notificationService.ts` + `src/lib/line.ts`) และ wire เข้า order/payment/stock จริงแล้ว (รายละเอียด → รอบ 4c ข้อ 8 ด้านบน) — ไฟล์ `src/lib/notify.ts` ที่ plan เดิมพูดถึงไม่เคยมีอยู่จริง |
