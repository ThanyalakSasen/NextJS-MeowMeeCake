...
| 3.3 | 🟡 `logger.ts` ✅ (2026-09-10) · `compensation.ts` ✅ helper + adopt preorderService (2026-09-11) · เหลือ persistOrder | **logger:** `src/lib/logger.ts` (JSON บรรทัดเดียว, level, `LOG_LEVEL`, serialize `err`) แทน `console.error` 5 จุด · `no-console` rule กันเพิ่ม · → [`infra-tooling.md`](infra-tooling.md) §2 · **compensation:** `src/lib/compensation.ts` `Saga` (`onRollback` / `rollback` reverse-order best-effort / `commit`) · adopt `preorderService.createPreorder` (แทน `committed[]` + nested try/catch + `.catch(()=>undefined)`) · → [`hardening-d3-plan.md`](hardening-d3-plan.md) · **ยังไม่ทำ:** refactor `orderService.persistOrder` / `updateOrderStatus` — รอ integration test ก่อน (D3.3) |
