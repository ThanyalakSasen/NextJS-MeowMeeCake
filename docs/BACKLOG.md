...
> **บั๊ก/ความเสี่ยงชุดใหม่ที่พบหลังไฟล์นี้ปิดครบ (ยังไม่แก้) → [`BACKLOG2.md`](BACKLOG2.md)**
...
| 3.8 | ~~สมุดที่อยู่ไม่เชื่อม checkout~~ | ✅ **แก้แล้ว** (2026-09-12, ตรวจซ้ำ 2026-09-13 — แถวนี้เดิมไม่ได้อัปเดตตามจริง) รายละเอียดเต็ม → รอบ 4c ข้อ 7 ด้านบน · `schemas/order.ts` รองรับ `address_id` + `.refine()` บังคับ oneOf กับ `delivery_address` แล้ว · **ยังไม่ทำ (gap ที่รู้ตัว):** `/api/shop/preorders` (POST) ยังเป็น path คู่ขนานที่ไม่รองรับ `address_id` |
