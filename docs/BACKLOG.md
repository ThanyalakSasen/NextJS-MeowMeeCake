...
| # | เรื่อง | หมายเหตุ |
|---|---|---|
| 3.1 | 🟡 validation layer (zod) — infra + adopt นำร่องแล้ว (2026-09-10) | `zod` v4 · `src/lib/validate.ts` (`parseBody`/`parseQuery`/`parse` → `badRequest(400, { issues })` เข้าทาง `route()` เดิม) · `src/schemas/common.ts` + `auth.ts` · adopt `POST /api/auth/register` + `/login` แล้ว · รายละเอียด + สถานะ adopt ราย route → [`validation.md`](validation.md) · **ยังไม่ทำ:** `/shop/orders` · `/shop/cart/*` · `/shop/payments` · admin orders · option `validate` ใน `crudRoutes` · รื้อ `pick()` ใน service |
