...
| # | เรื่อง | หมายเหตุ |
|---|---|---|
| 3.1 | 🟡 validation layer (zod) — infra + `/auth/*` + `/shop/*` + crudRoutes option (2026-09-11) | `zod` v4 · `src/lib/validate.ts` (`parseBody`/`parseQuery`/`parse` → `badRequest(400, { issues })` เข้าทาง `route()` เดิม) · `src/schemas/` (`common`, `auth`, `order`, `cart`, `payment`, `catalog`) · adopt: `/api/auth/register`+`/login`, `/api/shop/orders`(POST+GET), `/api/shop/cart/items`(+`/[id]`), `/api/shop/payments`(+`/[id]/slip`) · **`crudRoutes` เพิ่ม option `validate: { create, update }`** — adopt `units`/`product-categories`/`banners` · 61 tests · รายละเอียด → [`validation.md`](validation.md) · **ยังไม่ทำ:** CRUD factory ที่เหลือ (แค่เพิ่ม schema+บรรทัด) · admin orders custom route · รื้อ `pick()`/`Number()` ใน service |
