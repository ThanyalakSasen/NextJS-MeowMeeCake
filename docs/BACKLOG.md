...
| # | เรื่อง | หมายเหตุ |
|---|---|---|
| 3.1 | 🟡 validation layer (zod) — infra + `/auth/*` + `/shop/*` adopt แล้ว (2026-09-10) | `zod` v4 · `src/lib/validate.ts` (`parseBody`/`parseQuery`/`parse` → `badRequest(400, { issues })` เข้าทาง `route()` เดิม) · `src/schemas/` (`common`, `auth`, `order`, `cart`, `payment`) · adopt: `/api/auth/register`+`/login`, `/api/shop/orders` (POST+GET), `/api/shop/cart/items`(+`/[id]`), `/api/shop/payments`(+`/[id]/slip`) · 53 tests · รายละเอียด + สถานะ adopt ราย route → [`validation.md`](validation.md) · **ยังไม่ทำ:** admin orders/CRUD · option `validate` ใน `crudRoutes` · รื้อ `pick()`/`Number()` ใน service |
