...
> สร้าง: 2026-09-13 · อัปเดตล่าสุด: 2026-09-22 (§12 ใหม่ — 2 ข้อพบระหว่างเชื่อม frontend เข้ากับ backend
> จริงรอบล่าสุด: รหัสผ่าน seed เปิดเผยใน repo สาธารณะ (action จริง ยังไม่ทำ) + การเลือกสูตรกำกวมใน
> `createProductionFromRound` เมื่อสินค้ามีหลายสูตร (ความเสี่ยงที่ยังไม่มีผลกระทบจริง ดู PR #44) — §1–§11
> เดิมยังปิดครบเหมือนเดิม ไม่มีอะไรเปลี่ยน)
...
## 12. 🔴 พบระหว่างเชื่อม frontend เข้ากับ backend จริง (2026-09-22) — ยังไม่ได้แก้
...
**การตัดสินใจ:** บันทึกเป็นความเสี่ยงไว้ก่อนตามรูปแบบเดียวกับ §9 — ยังไม่ถามผู้ใช้ว่าจะแก้แบบไหน (ตัวเลือก
ที่เป็นไปได้: sort `created_at: -1` เอาสูตรล่าสุดเหมือน `getUnitCostByProduct()`, หรือ 400 ปฏิเสธตรง ๆ ถ้า
เจอสินค้าที่มีสูตรซ้ำ กันสร้างใบสั่งผลิตด้วยสูตรที่ไม่ตั้งใจ) เพราะยังไม่มีจุดพังจริงให้ทดสอบ

**เงื่อนไขที่ควรกลับมาทำ:** ก่อนอนุญาตให้สินค้าหนึ่งมีสูตรที่ใช้งานพร้อมกันหลายสูตร (เช่น สูตรทางเลือกตาม
ฤดูกาล/ต้นทุนวัตถุดิบ) ต้องตัดสินใจ + แก้เรื่องนี้ก่อน ไม่งั้นใบสั่งผลิตที่สร้างจากรอบพรีออเดอร์อาจอ้างอิง
สูตรที่ไม่ตรงกับที่ตั้งใจ

**สถานะ:** 🟡 ไม่แก้โค้ดตอนนี้ — บันทึกเป็น known risk (ดู PR #44 test plan ที่พูดถึงจุดนี้ไว้แล้วเช่นกัน)
