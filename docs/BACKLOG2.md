...
## 15. ✅ `banner_img` เก็บเป็น base64 ตรง ๆ ทำให้เพิ่ม/แก้แบนเนอร์ด้วยรูปจริงพัง 400 เสมอ (พบ + แก้ 2026-09-23)

> พบระหว่างผู้ใช้ทดสอบเพิ่มแบนเนอร์จริงผ่านเบราว์เซอร์ — เจอ `POST /admin/banners` ตอบ `400` ทุกครั้งที่
> แนบรูปจริง (ไม่เกี่ยวกับงาน `product_type` ที่ทำค้างอยู่คนละเรื่องกันเลย — ยืนยันแล้วว่า `bannersModel`
> ไม่มีความสัมพันธ์กับ `product_type`/`product_types` เลย)

**สาเหตุ:** `schemas/catalog.ts` `bannerCreate.banner_img` จำกัดไว้แค่ `max(1000)` ตัวอักษร — ตั้งใจไว้
สำหรับ URL ธรรมดา แต่ frontend (`UploadImageBox`) เก็บรูปเป็น **base64 data URI ตรง ๆ** (ไม่เคยอัปโหลด
ไฟล์จริงเลยตั้งแต่ต้น ต่างจาก `product_img` ที่แก้ไปแล้วก่อนหน้านี้) รูปภาพจริงทุกรูปเข้ารหัส base64 แล้ว
ยาวเกิน 1000 ตัวอักษรเสมอ (ยืนยันซ้ำด้วยเบราว์เซอร์จริง: อัปโหลดรูป 2MB → error
`"Too big: expected string to have <=1000 characters"`)

**แก้แล้ว — ย้ายไปอัปโหลดไฟล์จริงเหมือน `product_img`** (ผู้ใช้ขอให้แก้ที่ต้นเหตุ ไม่ใช่แค่ขยายเพดาน):
- เพิ่ม `POST /api/admin/banners/images` (สิทธิ์ `products.update` เหมือน `/admin/banners` เอง) —
  ใช้ `saveImages(files, "banners")` ตัวเดียวกับ products, เก็บไฟล์จริงใน
  `public/uploads/banners/` คืน `{ url, filename, size }`
- ลดเพดาน `banner_img` กลับเป็น `max(500)` (พอสำหรับ URL path ปกติ เช่น `/uploads/banners/xxx.jpg`)
- `bannerService.ts` เพิ่ม `update()` ที่ลบไฟล์รูปเดิมทิ้งเมื่อแทนที่ด้วยรูปใหม่ (แพทเทิร์นเดียวกับ
  `productService.ts` BACKLOG §3.14 — ต่างกันที่แบนเนอร์มีรูปเดียว ไม่ใช่ array) ไม่ลบไฟล์ตอน soft
  delete/restore เพราะยังกู้คืนได้

**ยืนยันด้วยเบราว์เซอร์จริงครบทุกจุด:**
- อัปโหลดรูป JPEG จริง → `200`, คืน `/uploads/banners/xxx.jpg`, ยืนยันไฟล์มีอยู่จริงบน disk
- สร้างแบนเนอร์ด้วย URL นั้น → `201` สำเร็จ
- แก้ `banner_img` เป็น URL อื่น → ไฟล์เดิมถูกลบออกจาก disk จริง (ยืนยันด้วย `ls` ก่อน/หลัง)
- แบนเนอร์เก่าที่ยังเป็น base64 อยู่ (สร้างไว้ก่อนแก้) ยังแสดงผลได้ปกติ (`resolveUploadUrl()` ปล่อยผ่าน
  `data:` URI อยู่แล้ว — ไม่ต้อง migrate ข้อมูลเก่า)

ยืนยันด้วย `typecheck` (0 error) ฝั่ง backend, `npm run check` (i18n+theme+tsc+eslint) ฝั่ง frontend
